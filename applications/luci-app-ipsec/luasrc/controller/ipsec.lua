...
function index()
	...
	entry({"admin", "network", "ipsec", "new"}, call("render_new"), nil)
	entry({"admin", "network", "ipsec", "new_manual"}, call("render_new_manual"), nil)
	entry({"admin", "network", "ipsec", "edit"}, call("render_edit"), nil)

	entry({"admin", "network", "ipsec", "start"}, call("action_start"), nil)
	entry({"admin", "network", "ipsec", "stop"}, call("action_stop"), nil)
	entry({"admin", "network", "ipsec", "status"}, call("action_status"), nil)
	entry({"admin", "network", "ipsec", "export"}, call("action_export"), nil)
end
...
function render_overview()
	...
	-- Render page
	luci.template.render("ipsec/overview", {
		rows = rows
	})
end
...
function render_new()
	...
	-- Render page
	luci.template.render("ipsec/new", {
		row = {},
...
function render_new_manual()
	...
	-- Render page
	luci.template.render("ipsec/new_manual", {
		row = {},
		local_id = get_local_id()
	})
end
...
function render_edit()
	...
	-- Render page
	luci.template.render("ipsec/edit", {
		row = row,
		local_id = get_local_id()
	})
end
